...
class CrossOver():
	...
	def __recombine(parent1, parent2, child1, child2):
	...
	def crossover(population, populationSize):
		...
		for i in range(0, populationSize // 2, 2):
			...
			totalPixel = len(population[i]["image"]["pixels"])
			...
			for i in range(0, crossoverPoint):
				population[i + populationSize // 2]["image"]["pixels"][i] = population[i]["image"]["pixels"][i]
				population[i + populationSize // 2 + 1]["image"]["pixels"][i] = population[i + 1]["image"]["pixels"][i]
			...
			for i in range(crossoverPoint, totalPixel):
				population[i + populationSize // 2 + 1]["image"]["pixels"][i] = population[i]["image"]["pixels"][i]
				population[i + populationSize // 2]["image"]["pixels"][i] = population[i + 1]["image"]["pixels"][i]
		return population
